...
NEWSREAP_PATH = join(dirname(dirname(abspath(__file__))))

try:
    from lib.NNTPDatabase import NNTPDatabase

except ImportError:
    sys.path.insert(0, NEWSREAP_PATH)
    from lib.NNTPDatabase import NNTPDatabase
...
DEFAULT_CLI_PLUGIN_DIRECTORIES = (
    # Main System
    join(NEWSREAP_PATH,'lib', 'plugins', 'cli'),
